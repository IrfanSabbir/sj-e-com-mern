...
import {  Paper, Button, Table, TableBody,
...
const ProductList = () => {
  ...
  const getProducts = async () => {
    try {
      ...
      const result = await axios.get(process.env.REACT_APP_BASE_URL +
        `admin_api/product/list/`, { headers: headers })
  ...
  const  createData= (product) =>{
    ...
    const thumbnail = <img src={process.env.REACT_APP_BASE_URL+product.thumbnail} width="70px" height="70px auto" style={{borderRadius:"50%", border:"1px solid blue"}}/>
    
    const action =<div style={{ display: "flex" }}>
      <Button color="success" onClick={() => navigate('/update/'+ product._id)}>Update</Button>
